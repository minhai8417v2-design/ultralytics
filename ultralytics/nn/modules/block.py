...
class DCN2Conv(nn.Module):
    def __init__(self, c1, c2, k=3, s=1, p=None, g=1, d=1, act=True, bias=False):
        super().__init__()
        self.groups = g  # Store groups for proper handling
        p = k // 2 if p is None else p
        
        # For grouped convolution, adjust channel calculations
        self.offset_conv = nn.Conv2d(c1, 2 * k * k * g, k, s, p, d, bias=True)
        self.mask_conv = nn.Conv2d(c1, k * k * g, k, s, p, d, bias=True)
...
class Bottleneck_DCN(nn.Module):
    def __init__(self, c1, c2, shortcut=True, g=1, k=(3, 3), e=0.5):
        ...
        c_ = int(c2 * e)
        self.cv1 = Conv(c1, c_, k[0], 1)
        self.cv2 = DCN2Conv(c_, c2, k[1], 1, g=g)  # Replace only the second conv
        self.add = shortcut and c1 == c2
