...
def bbox_in_Wiou(box1, box2, xywh=True, CIoU=False, InCIoU=False, WIoU=False, InWIoU=False, scale=False, eps=1e-7, ratio=1.0):
    # Get the coordinates of bounding boxes
    if xywh:  # transform from xywh to xyxy
        (x1, y1, w1, h1), (x2, y2, w2, h2) = box1.chunk(4, -1), box2.chunk(4, -1)
        w1_, h1_, w2_, h2_ = w1 / 2, h1 / 2, w2 / 2, h2 / 2
        ...
        b1_x1, b1_y1, b1_x2, b1_y2 = box1.chunk(4, -1)
        b2_x1, b2_y1, b2_x2, b2_y2 = box2.chunk(4, -1)
        w1, h1 = b1_x2 - b1_x1, (b1_y2 - b1_y1) + eps   # .clamp(eps)    + eps
        w2, h2 = b2_x2 - b2_x1, (b2_y2 - b2_y1) + eps       # .clamp(eps)       + eps
    ...
    if scale:
        #self = WIoU_Scale(1 - innner_iou)
        self = WIoU_Scale(1.0 - (inter / union))
    # IoU
    iou = inter / (union ) #  + eps
    if CIoU or WIoU or InWIoU or InCIoU:
        cw = b1_x2.maximum(b2_x2) - b1_x1.minimum(b2_x1)  # convex (smallest enclosing box) width
        ch = b1_y2.maximum(b2_y2) - b1_y1.minimum(b2_y1)  # convex height
        ...
        if CIoU:
            ...
            with torch.no_grad():
                alpha_ciou = v / (v - iou + (1 + eps))
            return innner_iou - (rho2 / c2 + v * alpha_ciou) # CIoU
        elif WIoU: 
            if scale:
                #print('WIoU')
                return getattr(WIoU_Scale, '_scaled_loss')(self), (1 - iou) * torch.exp((rho2 / c2)), iou # WIoU https://arxiv.org/abs/2301.10051   (1 - innner_iou)**2
            else:
                return iou, torch.exp((rho2 / c2)) # WIoU v1
        elif InWIoU: 
            if scale:
                #print('InWIoU')
                return getattr(WIoU_Scale, '_scaled_loss')(self), ((1 - innner_iou)) * torch.exp((rho2 / c2)), innner_iou # In-WIoU,使用 平方   **2
            else:
